...
function createStackedAreaChartWithTooltip(optionalColorSchema) {
    let stackedArea = stackedAreaChart(),
        ...
        container = d3Selection.select(
            ".js-stacked-area-chart-tooltip-container"
        ),
    ...
    if (containerWidth) {
        ...
        const generateFooter = drawingInfo => {
            drawingInfo.tooltipFooterContainer
                .append("line")
                .classed("tooltip-divider", true)
                .attr("x1", -drawingInfo.tooltipWidth / 4 + 16)
                .attr("x2", drawingInfo.tooltipWidth - 60)
                .attr("y1", drawingInfo.ttTextY)
                .attr("y2", drawingInfo.ttTextY)
                .style("stroke", drawingInfo.borderStrokeColor);

            drawingInfo.ttTextY += 10;
            drawingInfo.tooltipHeight += 10;

            const textElem = drawingInfo.tooltipFooterContainer
                .append("text")
                .classed("tooltip-footer-text", true)
                .attr("dy", "1em")
                .attr("x", -drawingInfo.tooltipWidth / 2)
                .attr("y", drawingInfo.ttTextY)
                .style("fill", drawingInfo.tooltipTextColor)
                .text("Footer")
                .call(
                    drawingInfo.textWrap,
                    drawingInfo.tooltipWidth - 60,
                    -drawingInfo.tooltipWidth / 4 + 16
                );

            drawingInfo.textHeight = textElem.node().getBBox().height
                ? textElem.node().getBBox().height
                : drawingInfo.textHeight;

            drawingInfo.tooltipHeight +=
                drawingInfo.textHeight + drawingInfo.tooltipTextLinePadding;
...
function createStackedAreaChartWithFixedAspectRatio(optionalColorSchema) {
    let stackedArea = stackedAreaChart(),
        chartTooltip = tooltip(),
        container = d3Selection.select(
            ".js-stacked-area-chart-fixed-container"
        ),
        containerWidth = container.node()
            ? container.node().getBoundingClientRect().width
            : false,
...
function createStackedAreaChartWithSyncedTooltip() {
    let stackedArea = stackedAreaChart(),
        chartTooltip = tooltip(),
        container = d3Selection.select(
            ".js-stacked-area-chart-tooltip-bis-container"
        ),
        containerWidth = container.node()
            ? container.node().getBoundingClientRect().width
            : false,
